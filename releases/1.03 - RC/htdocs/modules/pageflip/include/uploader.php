...
class PageflipXoopsMediaUploader
{
	var $mediaName;
	var $mediaType;
	var $mediaSize;
	 ...
	 **/
	function PageflipXoopsMediaUploader($uploadDir, $allowedMimeTypes, $maxFileSize, $maxWidth=null, $maxHeight=null, $allowedExtensions=null )
	{
		...
		if (!file_exists($uploadDir)) {
			$path = '';
			foreach(explode('\\', $uploadDir) as $folder) {
				foreach(explode('/', $folder) as $folderb) {
					$path .= DIRECTORY_SEPARATOR . $folderb;
					mkdir($path, 0777);
				}
			}
		}
	...
	function upload($chmod = 0644)
	{
		...
		if (!$this->checkMimeType()&&strlen($this->mediaType)>0) {
	...
	{
		if (!isset($this->maxHeight)||$this->maxHeight==0) {
			return true;
		}
		if (false !== $dimension = getimagesize($this->mediaTmpName)) {
