		...
		case "pages":	
			switch ($fct)
			{
				...
				case "save":
					...
					if ($uploader->fetchMedia('imagefile')) {
					  	if (!$uploader->upload()) {
					    	if ($pages->isNew()) {	
					      	...
					      	if (!$pages->isNew())
					      		unlink($pages->getVar('path').(substr($pages->getVar('path'), strlen($pages->getVar('path'))-1, 1)!=DIRECTORY_SEPARATOR?DIRECTORY_SEPARATOR:'').$pages->getVar('filename'));
					      	
					      	$pages->setVar('path', $GLOBALS['xoopsModuleConfig']['uploaddir']);
					...
					if (!$id=$pages_handler->insert($pages)) {
				...
				case "list":				
					...
					foreach($chapterss as $cid => $chapters) {
						if (is_object($chapters))
							$GLOBALS['pfTpl']->append('chapterss', $chapters->toArray());
					}
			...
			switch ($fct)
			{
				...
				case "list":				
					...
					foreach($csss as $cid => $css) {
						if (is_object($css))
							$GLOBALS['pfTpl']->append('csss', $css->toArray());
					}
