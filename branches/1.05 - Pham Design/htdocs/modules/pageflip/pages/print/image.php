	...
	if (file_exists($image)) {
		switch($pages->getVar('filetype')) {
			...
			case '_MI_PAGEFLIP_FILETYPE_GIF':
				...
				$ratio_width = $pages->getVar('width') / $book->getVar('bookWidth');
				$ratio_height = $pages->getVar('height') / $book->getVar('bookHeight'); 
				...
				if (file_exists($image)) {
					...
					if ($GLOBALS['xoopsModuleConfig']['scale_images']&&$GLOBALS['xoopsModuleConfig']['auto_crop_images']) {
						$newImage = $img->resize($book->getVar('bookWidth')*$scale, $book->getVar('bookHeight')*$scale)->crop("center", "middle", $book->getVar('bookWidth'), $book->getVar('bookHeight'));
						ob_end_clean();
						$newImage->output($format, 99);
						exit(0);
					} elseif ($GLOBALS['xoopsModuleConfig']['scale_images']&&!$GLOBALS['xoopsModuleConfig']['auto_crop_images']) { 
						$newImage = $img->resize($book->getVar('bookWidth'), $book->getVar('bookHeight'));
						ob_end_clean();
						$newImage->output($format, 99);
						exit(0);
					} else {
